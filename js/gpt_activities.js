...
if (details.auth) {
    html += `<div class="info-row">
        ...
        // فحص القرار 104
        const decision104Info = checkDecision104Full(activity.text);
        if (decision104Info) {
    ...
    function formatGuideInfo(details) {
        ...
        if (!html) {
            return "⚠️ معلومات الدليل غير متوفرة حالياً.";
        }
    ...
    function formatSuitableLocation(details) {
        ...
        return `<div class="info-card" style="background: linear-gradient(135deg, #fce4ec 0%, #f8bbd0 100%); border-left-color: #c2185b;">
            ...
            <div class="info-card-content" style="color: #ad1457;">
                ${details.loc}
            </div>
        </div>`;
    }
